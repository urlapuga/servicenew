<!doctype html>
<html>
<head>
</head>
<body>

<strong>
    <p align="right">Додаток №1<br>
        до Договору № 13  від «31серпня» 2017 року</p>

    <table width="100%">
        <tr width="100%">
            <td align="left">
                м. Київ
            </td>
            <td align="right">
                «31» серпня 2017 року
            </td>
        </tr>
    </table>
                                                                                             
    <p align="center">Специфікація  Послуг</p>
    <p align="center">1.  Послуги по організації некомутованого з&rsquo;єднання</p>


</strong>
<table border="1" cellspacing="0" cellpadding="0" width="100%">
    <tr height="25">
        <th align="center">Пункт</th>
        <th align="center">Назва послуги</th>
        <th align="center">Вартість послуги, грн.</th>
    </tr>
    <tr height="25">
        <td align="center">1.1.</strong></td>
        <td align="center">Організація фізичного некомутованого з&rsquo;єднання за адресою: м. Київ, просп. Степана Бандери, 23</td>
        <td align="center"></td>
    </tr>
    <tr height="25">
        <td align="center"></td>
        <td align="right">ПДВ    20%<br>
            Всього    до сплати:</td>
        <td  align="center"></td>
    </tr>
</table>



<p><strong>Розділ 2. Доступ до мережі Інтернет без  обмеження по трафіку 100 Мбіт/с.</strong></p>

<table border="1" cellspacing="0" cellpadding="0" width="100%">
    <tr height="25">
        <th align="center">Пункт</th>
        <th align="center">Назва послуги</th>
        <th align="center">Вартість послуги, грн.</th>
    </tr>
    <tr height="25">
        <td align="center"><strong>1.1.</strong></td>
        <td align="center">Абонплата за    місяць (швидкість порту _______Мбіт/с)</td>
        <td align="center"></td>
    </tr>
    <tr height="25">
        <td align="center"><strong>1.2.</strong></td>
        <td align="center">Статичний    IP    адрес</td>
        <td align="center"></td>
    </tr>
    <tr height="25">
        <td align="center"></td>
        <td align="center"></td>
        <td align="center"></td>
    </tr>
    <tr height="25">
        <td align="center"></td>
        <td align="right"><strong>ПДВ    20% <br>
            Всього    до сплати:</strong></td>
        <td align="center"></td>
    </tr>
</table>


<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<table border="0" cellspacing="0" cellpadding="0" width="704">
    <tr>
        <td width="345" valign="top"><p><strong>____________________________________</strong> <br>
            ____________________________________<br>
            ____________________________________ <br>
            ____________________________________<br>
            ____________________________________<br>
            ____________________________________ </p>
            <p><strong>______________________</strong></p>
            <p><strong>______________    _________________</strong></p></td>
        <td width="359" valign="top"><p><strong>ТОВ    «САКСКАЙ»</strong><strong> </strong><br>
            04655,    м. Київ, пр. Московський (Степана Бандери), 23<br>
            Код    ЄДРПОУ 38883067 <br>
            р/р  26003052750322<br>
            у    Печерській філії ПАТ КБ «ПРИВАТБАНК» у м. Києві,<br>
            МФО    300711 </p>
            <p><strong>Директор </strong></p>
            <p><strong>______________ САДОВСЬКИЙ О.Л.</strong></p></td>
    </tr>
</table>





</body>
</html>